# Intégrer l'envoi de courrier papier — API Merci Facteur

> Document de référence à donner à un assistant de développement (Claude, ChatGPT, Cursor, Copilot, Lovable, v0…).
> Il décrit **comment déclencher un envoi de courrier** avec l'API Merci Facteur. Il ne prescrit aucune architecture :
...
L'API Merci Facteur permet d'envoyer un vrai courrier papier — imprimé et posté par La Poste — depuis une application. Ce document couvre l'envoi de **lettres au format PDF**. L'API sait aussi envoyer des cartes, des photos, des recommandés électroniques et des publipostages : voir la spec.
...
Récupère donc le PDF depuis ton propre serveur, encode-le, et envoie-le dans `base64files`. Le base64 ne contient ni retour à la ligne, ni préfixe `data:application/pdf;base64,`, ni marqueur de fin.
